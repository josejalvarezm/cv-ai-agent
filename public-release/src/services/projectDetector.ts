 ...
 * 
 * Identifies project-specific queries (e.g., "skills at Acme Corp")
...
export class ProjectDetectorService implements IProjectDetector {
  private readonly projectPatterns: ProjectPattern[] = [
    // Add your employer-specific patterns here
    // Example: { pattern: /\b(at|in|for|during|with)\s+(acme|acme.*corp)\b/i, name: 'Acme Corp' },
    // Example: { pattern: /\b(acme)\b/i, name: 'Acme Corp' },
    { pattern: /\b(at|in|for|during|with)\s+(techstart|tech.*start)\b/i, name: 'TechStart' },
    { pattern: /\b(techstart)\b/i, name: 'TechStart' },
  ];
  ];
