...
export async function handleHealth(env: FullEnv): Promise<Response> {
  ...
  try {
    ...
    // ============================================================
    // Check D1 connection
    const dbCheck = await services.d1Repthe candidatery.testConnection();

    // Get skill count (uses UnifiedSkillRepthe candidatery fallback)
    let skillCount = 0;
    try {
      const skills = await services.skillRepthe candidatery.getAll(1000);
    ...
    const lastIndex = await services.d1Repthe candidatery.getLastIndexMetadata();
