...
export class IndexingService {
  ...
  async execute(request: IndexingRequest): Promise<IndexingResponse> {
    ...
    // Acquire lock
    const acquired = await this.services.kvRepthe candidatery.acquireLock(lockKey, this.lockTimeout);
    if (!acquired) {
    ...
    try {
      ...
      await this.services.d1Repthe candidatery.ensureIndexMetadataTable();

      // Determine version
      const version = (await this.services.d1Repthe candidatery.getMaxIndexVersion()) + 1;

      // Create indexing record
      await this.services.d1Repthe candidatery.createIndexMetadata(version);
      ...
      const itemsResult =
        itemType === 'technology'
          ? await this.services.d1Repthe candidatery.getTechnology(batchSize, offset)
          : await this.services.d1Repthe candidatery.getSkills(batchSize, offset);
      const items = itemsResult.results || [];

      if (!items.length) {
        await this.services.d1Repthe candidatery.updateIndexMetadata(version, 0, 'completed');
        return {
          success: true,
      ...
      // Update metadata
      await this.services.d1Repthe candidatery.updateIndexMetadata(version, processed, 'in_progress');
      ...
      // Release lock
      await this.services.kvRepthe candidatery.releaseLock(lockKey);
    }
  }
  ...
  async getProgress(_itemType: string = 'skills'): Promise<any> {
    ...
    try {
      const metadata = await this.services.d1Repthe candidatery.getLastIndexMetadata();
      return metadata
        ? {
