...
export class ProjectDetectorService implements IProjectDetector {
  private readonly projectPatterns: ProjectPattern[] = [
    { pattern: /\b(at|in|for|during|with)\s+(Acme Corp|conservative.*hq|conservative.*party)\b/i, name: 'Acme Corp' },
    { pattern: /\b(Acme Corp)\b/i, name: 'Acme Corp' },
    { pattern: /\b(at|in|for|during|with)\s+(TechStart)\b/i, name: 'TechStart' },
